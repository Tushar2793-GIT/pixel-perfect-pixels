...
const AudienceVisual = ({ title }: { title: string }) => {
  ...
  if (title === "For Non-Traders") {
    return <div className="relative h-56 w-full overflow-hidden rounded-lg bg-gradient-soft"><Bot className="absolute left-12 top-7 size-24 animate-float text-primary" /><TrendingUp className="absolute bottom-6 right-6 size-36 text-success" /><div className="absolute bottom-7 left-20 h-16 w-10 rounded-t bg-success/25" /><div className="absolute bottom-7 left-36 h-24 w-10 rounded-t bg-success/35" /><div className="absolute bottom-7 left-52 h-32 w-10 rounded-t bg-success/55" /></div>;
  }
  return <div className="relative flex h-56 w-full items-center justify-center"><div className="absolute size-44 rounded-full border-2 border-primary/25" /><BriefcaseBusiness className="size-24 text-primary" />{Array.from({ length: 6 }).map((_, i) => <Users key={i} className="absolute size-10 rounded-full bg-secondary p-2 text-primary" style={{ left: `${48 + Math.cos((i / 6) * Math.PI * 2) * 78}px`, top: `${86 + Math.sin((i / 6) * Math.PI * 2) * 78}px` }} />)}</div>;
};
...
const PlatformPreview = () => (
  <div className="mt-5 grid items-center gap-6 rounded-xl border bg-card p-6 shadow-sm xl:grid-cols-[280px_1fr_280px]">
    ...
    <div className="grid overflow-hidden rounded-xl bg-brand-navy p-4 text-primary-foreground shadow-card lg:grid-cols-[1.18fr_0.82fr]">
      <div><div className="mb-3 flex flex-wrap items-center gap-3 text-xs font-extrabold text-primary-foreground/70"><span>NIFTY 50</span><ChevronDown className="size-4" /><span>1m</span><span>5m</span><span>15m</span><span>1H</span><span>D</span><span>W</span><span>M</span><span>Indicators</span></div><h3 className="text-3xl font-extrabold text-success">24,813.55</h3><p className="font-extrabold text-success">+110.25 (0.45%)</p><MiniChart dark compact /></div>
      <div className="grid gap-3 p-3"><div className="rounded-lg border border-primary-foreground/10 p-4"><h3 className="font-extrabold">Positions (6)</h3><p className="mt-3 text-success">Total P&L +₹ 24,530.75</p>{["RELIANCE +₹ 6,210.00", "BANKNIFTY +₹ 5,320.00", "TCS -₹ 1,150.00", "INFY +₹ 2,450.00"].map((x) => <p key={x} className={`mt-3 text-sm font-bold ${x.includes("-") ? "text-destructive" : "text-success"}`}>{x}</p>)}<div className="mt-4 flex flex-wrap gap-3"><Button variant="brand" size="sm">New Order</Button><Button variant="brandOutline" size="sm" className="bg-transparent text-primary-foreground">Strategy Builder</Button></div></div><div className="rounded-lg border border-primary-foreground/10 p-4"><h3 className="font-extrabold">Risk Summary</h3><div className="mt-4 flex items-center gap-4"><div className="size-20 rounded-full border-[14px] border-success border-r-brand-gold border-b-brand-orange" /><div className="space-y-2 text-xs font-bold"><p>Low Risk 60%</p><p>Medium Risk 30%</p><p>High Risk 10%</p></div></div><p className="mt-4 text-sm font-bold">Daily Risk Limit ₹ 50,000</p><div className="mt-2 h-3 overflow-hidden rounded-full bg-primary-foreground/15"><div className="h-full w-[37%] bg-success" /></div></div></div>
    </div>
...
const Index = () => (
  <main id="top" className="min-h-screen bg-background text-foreground">
    ...
    <section className="mx-auto max-w-[1920px] px-5 py-9 lg:px-8 2xl:px-10">
      ...
      <div className="mt-6 grid items-center gap-6 overflow-hidden rounded-xl bg-secondary px-8 py-7 lg:grid-cols-[280px_1fr_auto]"><AnimatedRobot /><div><h2 className="font-display text-4xl font-extrabold text-brand-navy">Ready to Take Your Trading to the Next Level?</h2><p className="mt-3 text-xl font-bold text-muted-foreground">Join FastTrade99 and experience the power of AI, automation and advanced risk management.</p></div><div className="flex flex-wrap gap-5"><Button variant="brand" className="h-16 px-10 text-xl font-extrabold">Start Your Free Trial</Button><Button variant="brandOutline" className="h-16 px-10 text-xl font-extrabold">Book a Demo</Button><p className="w-full text-center text-base font-extrabold text-muted-foreground">No Credit Card Required • 7-Day Free Trial</p></div></div>
    </section>
