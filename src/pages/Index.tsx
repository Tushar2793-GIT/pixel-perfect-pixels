...
const RequestDemoSection = () => {
  ...
  const inputCls = "h-12 w-full rounded-md border border-border bg-card px-4 text-base font-semibold text-brand-navy placeholder:text-muted-foreground/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary";
  const labelCls = "mb-2 block text-sm font-extrabold uppercase tracking-wide text-primary";

  return (
    <section id="request-demo" className="relative overflow-hidden bg-gradient-soft px-5 py-16 lg:px-8 2xl:px-10">
      ...
      <div className="relative mx-auto max-w-[1100px] text-center">
        <p className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-4 py-1.5 text-sm font-extrabold uppercase tracking-widest text-primary">
          <Sparkles className="size-4" /> Get Started
        </p>
        <h2 className="mt-5 font-display text-4xl font-extrabold leading-tight text-brand-navy sm:text-5xl">
          Request a <span className="bg-gradient-brand bg-clip-text text-transparent">Live Demo</span>
        </h2>
        <p className="mx-auto mt-4 max-w-[680px] text-lg font-bold leading-8 text-muted-foreground">
          Speak with our team to see FastTrade99 in action — live execution, risk controls, and a full platform walkthrough.
        </p>

        <form onSubmit={handleSubmit} className="mx-auto mt-10 grid gap-5 rounded-2xl border border-primary/15 bg-card p-7 text-left shadow-card sm:p-9 md:grid-cols-2">
          <div>
            <label className={labelCls}>Full Name *</label>
            <Input className={inputCls} placeholder="Rajesh Kumar" value={form.name} onChange={(e) => update("name", e.target.value)} maxLength={100} />
          </div>
          <div>
            <label className={labelCls}>Phone Number *</label>
            <Input className={inputCls} placeholder="+91 98765 43210" value={form.phone} onChange={(e) => update("phone", e.target.value)} maxLength={20} />
          </div>
          <div className="md:col-span-2">
            <label className={labelCls}>Email Address *</label>
            <Input type="email" className={inputCls} placeholder="rajesh@example.com" value={form.email} onChange={(e) => update("email", e.target.value)} maxLength={255} />
          </div>
          <div>
          ...
          <div className="md:col-span-2">
            <Button type="submit" variant="brand" size="lg" className="h-14 w-full rounded-md text-lg font-extrabold shadow-button">
              <Send className="size-5" /> Schedule Live Demo
            </Button>
            <p className="mt-3 text-center text-sm font-bold text-muted-foreground">
              <CalendarCheck className="mr-1 inline size-4 text-success" /> Personalised walkthrough · No credit card required
            </p>
          </div>
        </form>
      </div>
