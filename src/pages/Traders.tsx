...
import traderAvatars from "@/assets/nontrader-investor-avatars.jpg";
...
const Traders = () => (
  <main className="min-h-screen bg-background text-foreground">
      ...
      <div className="grid min-h-[610px] overflow-hidden rounded-xl border bg-card shadow-sm lg:grid-cols-[0.34fr_0.66fr]">
        <div className="flex flex-col justify-center px-7 py-9 xl:px-10">
          ...
          <div className="absolute inset-y-0 left-0 w-52 bg-gradient-to-r from-card via-card/70 to-transparent" />
            <div className="absolute right-5 top-5 w-[min(390px,calc(100%-2.5rem))] rounded-xl border border-primary-foreground/10 bg-brand-navy/90 p-6 text-primary-foreground shadow-card backdrop-blur">
            ...
            <p className="mt-5 text-base font-bold text-primary-foreground/70">Total P&L</p><p className="mt-2 font-display text-[40px] font-extrabold text-success">+10.68%</p><p className="mt-1 text-[26px] font-extrabold leading-tight">₹42,72,380</p><p className="mt-4 text-base font-extrabold text-primary-foreground/70">vs Last Month <span className="text-success">+9.21%</span></p>
          ...
          <div className="absolute bottom-5 right-5 grid w-[min(470px,calc(100%-2.5rem))] grid-cols-2 gap-3 text-primary-foreground"><div className="min-w-0 rounded-xl border border-primary-foreground/10 bg-brand-navy/90 p-5 backdrop-blur"><p className="font-extrabold text-primary-foreground/80">Capital Deployed</p><p className="mt-4 break-words text-[clamp(1.45rem,1.65vw,1.75rem)] font-extrabold leading-tight">₹4,12,35,450</p><p className="mt-4 font-extrabold text-success">+18.45%</p></div><div className="min-w-0 rounded-xl border border-primary-foreground/10 bg-brand-navy/90 p-5 backdrop-blur"><p className="font-extrabold text-primary-foreground/80">Active Accounts</p><p className="mt-4 text-[clamp(1.45rem,1.65vw,1.75rem)] font-extrabold leading-tight">99+</p><p className="mt-4 font-extrabold text-success">+12 This Month</p></div></div>
        </div>
      </div>
    </section>
    ...
    <section className="mx-auto max-w-[1920px] px-6 py-2 2xl:px-8">
      <div className="rounded-xl bg-card py-6">
        ...
        <div className="mt-7 grid gap-6 xl:grid-cols-4">
          {workSteps.map(([Icon, title, bullets, tone], index) => { const StepIcon = Icon as typeof ShieldCheck; return <article key={title as string} className={`relative flex min-h-[360px] flex-col justify-between rounded-xl border p-7 shadow-sm ${tone === "success" ? "bg-success/5" : "bg-secondary/35"}`}>{index < workSteps.length - 1 && <ArrowRight className="absolute -right-5 top-1/2 hidden size-9 -translate-y-1/2 text-primary xl:block" />}<div className="flex items-start justify-between gap-4"><StepIcon className={`size-20 shrink-0 ${tone === "success" ? "text-success" : "text-primary"}`} /><span className={`flex size-11 shrink-0 items-center justify-center rounded-full text-xl font-extrabold text-primary-foreground ${tone === "success" ? "bg-success" : "bg-gradient-brand"}`}>{index + 1}</span></div><h3 className="mt-5 text-[clamp(1.35rem,1.45vw,1.75rem)] font-extrabold leading-tight text-brand-navy">{title as string}</h3><ul className="mt-5 space-y-3">{(bullets as string[]).map((item) => <li key={item} className="flex gap-3 text-[15px] font-bold leading-6 text-muted-foreground"><Check className={`mt-1 size-4 shrink-0 ${tone === "success" ? "text-success" : "text-primary"}`} /><span>{item}</span></li>)}</ul></article>; })}
        </div>
      </div>
    </section>
    ...
    <section className="mx-auto grid max-w-[1920px] items-stretch gap-4 px-6 py-2 2xl:px-8 2xl:grid-cols-[0.46fr_0.27fr_0.27fr]">
      <div className="flex min-h-[410px] flex-col rounded-xl border bg-card p-6 shadow-sm"><h2 className="font-display text-[26px] font-extrabold text-brand-navy">Strategy Intelligence</h2><div className="mt-6 grid flex-1 gap-6 lg:grid-cols-[0.54fr_0.46fr]"><div className="grid content-between gap-5">{intelligence.map(([Icon, title, text]) => { const IntelIcon = Icon as typeof ShieldCheck; return <article key={title as string} className="grid grid-cols-[72px_1fr] items-center gap-5"><span className="flex size-16 shrink-0 items-center justify-center rounded-full bg-success/10 text-success"><IntelIcon className="size-8" /></span><div className="min-w-0"><h3 className="text-[18px] font-extrabold leading-6 text-brand-navy">{title as string}</h3><p className="mt-2 text-[15px] font-bold leading-6 text-muted-foreground">{text as string}</p></div></article>; })}</div><div className="relative flex min-h-[320px] flex-col overflow-hidden rounded-xl bg-brand-navy p-7 text-primary-foreground shadow-card"><h3 className="text-2xl font-extrabold">Our Core Edge</h3><ul className="mt-5 space-y-4">{["AI Powered Market Analysis", "High Probability Setups Only", "Dynamic Strategies", "Strict Risk Management", "Consistent 10–12% Monthly Returns"].map((item) => <li key={item} className="flex items-center gap-4 text-base font-extrabold leading-6"><Check className="size-5 shrink-0 text-success" />{item}</li>)}</ul><div className="pointer-events-none absolute inset-x-0 bottom-0 h-36 bg-[radial-gradient(ellipse_at_center,hsl(var(--primary)/0.45),transparent_62%)]" /><div className="relative mx-auto mt-auto flex size-24 rotate-45 items-center justify-center rounded-2xl bg-primary shadow-button ring-2 ring-primary-glow/50"><span className="-rotate-45 font-display text-3xl font-extrabold text-primary-foreground">AI</span></div></div></div></div>
      <div className="flex min-h-[410px] flex-col rounded-xl border bg-card p-6 shadow-sm"><h2 className="font-display text-[26px] font-extrabold text-brand-navy">Live Market Pulse <span className="text-base text-muted-foreground">(AI View)</span></h2><div className="mt-5 flex flex-1 flex-col justify-between">{[["NIFTY 50", "22,530.70"], ["BANK NIFTY", "48,125.40"]].map(([name, value]) => <div key={name} className="border-b pb-4"><p className="text-lg font-extrabold text-brand-navy">{name}</p><div className="mt-2 flex items-center justify-between gap-4"><p className="font-display text-[clamp(1.65rem,1.75vw,2rem)] font-extrabold text-brand-navy">{value}</p><svg viewBox="0 0 260 58" className="h-14 w-[42%] min-w-32 shrink-0"><path d="M4 38 L18 18 L31 30 L45 21 L58 25 L72 20 L86 16 L100 30 L114 20 L128 25 L142 15 L156 8 L170 22 L184 29 L198 23 L212 31 L226 18 L240 12 L256 19" fill="none" stroke="hsl(var(--success))" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" /></svg></div></div>)}<div className="grid grid-cols-[1fr_96px] items-center gap-4 border-b pb-4"><div><p className="text-lg font-extrabold text-brand-navy">Market Momentum</p><p className="mt-1 font-display text-[34px] font-extrabold leading-none text-success">Strong</p><p className="mt-1 font-bold text-muted-foreground">Bullish Bias</p></div><div className="relative flex size-24 items-center justify-center rounded-full border-4 border-success/15"><span className="absolute size-16 rounded-full border-2 border-success/20" /><span className="absolute size-10 rounded-full border-2 border-success/30" /><span className="size-4 rotate-45 bg-success" /></div></div><div><div className="flex items-center justify-between gap-4 text-base font-extrabold text-brand-navy"><span>AI Confidence Score</span><span>85%</span></div><div className="mt-3 h-3 rounded-full bg-secondary"><div className="h-full w-[85%] rounded-full bg-success" /></div></div></div></div>
      <div className="flex min-h-[410px] flex-col rounded-xl border bg-card p-6 shadow-sm"><h2 className="font-display text-[26px] font-extrabold text-brand-navy">Capital Allocation <span className="text-base text-muted-foreground">(This Month)</span></h2><div className="mt-5 flex flex-1 flex-col justify-between"><div className="grid items-center gap-6 sm:grid-cols-[210px_1fr]"><div className="relative size-52 shrink-0 rounded-full" style={{ background: "conic-gradient(hsl(var(--primary)) 0 40%, hsl(var(--success)) 40% 70%, hsl(var(--brand-violet)) 70% 90%, hsl(var(--brand-orange)) 90% 100%)" }}><div className="absolute inset-12 flex flex-col items-center justify-center rounded-full bg-card text-center"><span className="font-display text-[24px] font-extrabold text-brand-navy">₹4.12Cr+</span><span className="text-sm font-bold text-muted-foreground">Total Deployed</span></div></div><ul className="min-w-0 space-y-4 text-base font-extrabold text-brand-navy">{[["Momentum Pro", "40%"], ["AI Swing", "30%"], ["Multi Average", "20%"], ["Alpha Breakout", "10%"]].map(([name, percent], i) => <li key={name} className="grid grid-cols-[1fr_auto] items-start gap-4"><span className="flex min-w-0 items-start gap-3"><span className={`mt-1.5 size-3 shrink-0 rounded-full ${i === 0 ? "bg-primary" : i === 1 ? "bg-success" : i === 2 ? "bg-brand-violet" : "bg-brand-orange"}`} /><span className="leading-6">{name}</span></span><span>{percent}</span></li>)}</ul></div><div className="mt-5 rounded-xl bg-secondary px-7 py-5 text-center"><p className="text-xl font-extrabold text-brand-navy">Diversified. Controlled. Optimized.</p><p className="mt-2 text-base font-bold text-muted-foreground">Built for consistent long-term growth.</p></div></div></div>
    </section>
