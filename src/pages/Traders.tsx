...
const Traders = () => (
  <main className="min-h-screen bg-background text-foreground">
    ...
    <section className="mx-auto grid max-w-[1920px] items-stretch gap-4 px-6 py-2 2xl:px-8 xl:grid-cols-[0.46fr_0.27fr_0.27fr]">
      <div className="flex h-full flex-col rounded-xl border bg-card p-8 shadow-sm"><h2 className="font-display text-[30px] font-extrabold text-brand-navy">Strategy Intelligence</h2><div className="mt-8 grid flex-1 gap-6 lg:grid-cols-[0.52fr_0.48fr]"><div className="flex flex-col justify-between gap-6">{intelligence.map(([Icon, title, text]) => { const IntelIcon = Icon as typeof ShieldCheck; return <article key={title as string} className="flex min-h-[120px] items-center gap-6"><span className="flex size-16 shrink-0 items-center justify-center rounded-full bg-success/10 text-success"><IntelIcon className="size-8" /></span><div><h3 className="text-lg font-extrabold text-brand-navy">{title as string}</h3><p className="mt-2 text-base font-bold leading-6 text-muted-foreground">{text as string}</p></div></article>; })}</div><div className="flex flex-col justify-between rounded-xl bg-brand-navy p-8 text-primary-foreground shadow-card"><div><h3 className="text-2xl font-extrabold">Our Core Edge</h3><ul className="mt-6 space-y-5">{["AI Powered Market Analysis", "High Probability Setups Only", "Dynamic Strategies", "Strict Risk Management", "Consistent 10–12% Monthly Returns"].map((item) => <li key={item} className="flex items-center gap-4 text-lg font-extrabold"><Check className="size-6 shrink-0 text-success" />{item}</li>)}</ul></div><div className="mx-auto mt-7 flex size-36 rotate-45 items-center justify-center rounded-3xl bg-primary/25 shadow-button ring-1 ring-primary/40"><span className="-rotate-45 font-display text-5xl font-extrabold text-primary-foreground">AI</span></div></div></div></div>
      <div className="flex h-full flex-col rounded-xl border bg-card p-8 shadow-sm"><h2 className="font-display text-[28px] font-extrabold text-brand-navy">Live Market Pulse <span className="text-base text-muted-foreground">(AI View)</span></h2><div className="mt-6 flex flex-1 flex-col justify-between gap-6">{[["NIFTY 50", "22,530.70"], ["BANK NIFTY", "48,125.40"]].map(([name, value]) => <div key={name} className="border-b pb-5"><p className="text-lg font-extrabold text-brand-navy">{name}</p><div className="mt-2 flex items-center justify-between gap-4"><p className="font-display text-3xl font-extrabold text-brand-navy">{value}</p><svg viewBox="0 0 190 50" className="h-12 w-44 shrink-0"><path d="M4 34 L18 18 L31 27 L45 20 L58 24 L72 13 L86 33 L100 22 L115 30 L130 18 L145 26 L160 10 L180 18" fill="none" stroke="hsl(var(--success))" strokeWidth="4" strokeLinecap="round" /></svg></div></div>)}<div><p className="text-lg font-extrabold text-brand-navy">Market Momentum</p><p className="mt-2 font-display text-[36px] font-extrabold text-success">Strong</p><p className="font-bold text-muted-foreground">Bullish Bias</p></div><div><div className="flex items-center justify-between text-base font-extrabold text-brand-navy"><span>AI Confidence Score</span><span>85%</span></div><div className="mt-3 h-3 rounded-full bg-secondary"><div className="h-full w-[85%] rounded-full bg-success" /></div></div></div></div>
      <div className="flex h-full flex-col rounded-xl border bg-card p-8 shadow-sm"><h2 className="font-display text-[28px] font-extrabold text-brand-navy">Capital Allocation <span className="text-base text-muted-foreground">(This Month)</span></h2><div className="mt-6 flex flex-1 flex-col justify-between"><div className="flex items-center gap-8"><div className="relative size-56 shrink-0 rounded-full" style={{ background: "conic-gradient(hsl(var(--primary)) 0 40%, hsl(var(--success)) 40% 70%, hsl(var(--brand-violet)) 70% 90%, hsl(var(--brand-orange)) 90% 100%)" }}><div className="absolute inset-14 flex flex-col items-center justify-center rounded-full bg-card text-center"><span className="font-display text-[30px] font-extrabold text-brand-navy">₹4.12Cr+</span><span className="font-bold text-muted-foreground">Total Deployed</span></div></div><ul className="flex-1 space-y-5 text-base font-extrabold text-brand-navy">{[["Momentum Pro", "40%"], ["AI Swing", "30%"], ["Multi Average", "20%"], ["Alpha Breakout", "10%"]].map(([name, percent], i) => <li key={name} className="flex justify-between gap-4"><span className="flex items-center gap-3"><span className={`size-3 shrink-0 rounded-full ${i === 0 ? "bg-primary" : i === 1 ? "bg-success" : i === 2 ? "bg-brand-violet" : "bg-brand-orange"}`} />{name}</span><span>{percent}</span></li>)}</ul></div><div className="mt-8 rounded-xl bg-secondary px-7 py-6 text-center"><p className="text-xl font-extrabold text-brand-navy">Diversified. Controlled. Optimized.</p><p className="mt-2 text-base font-bold text-muted-foreground">Built for consistent long-term growth.</p></div></div></div>
    </section>
