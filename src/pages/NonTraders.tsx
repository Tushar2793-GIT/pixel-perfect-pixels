  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
        ...
        <div className="grid min-h-[610px] overflow-hidden rounded-xl border bg-card shadow-sm lg:grid-cols-[0.31fr_0.69fr]">
          <div className="flex flex-col justify-center px-6 py-8 lg:px-7 xl:px-9">
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              <Button variant="brand" className="h-16 min-w-0 rounded-md px-4 text-[15px] font-extrabold shadow-button xl:text-base">Start Investing Now <ArrowRight className="size-5 shrink-0" /></Button>
              <Button variant="brandOutline" className="h-16 min-w-0 rounded-md px-4 text-[15px] font-extrabold text-primary xl:text-base">How It Works <span className="flex size-9 shrink-0 items-center justify-center rounded-full border-2 border-primary"><Play className="ml-0.5 size-4 fill-primary" /></span></Button>
            </div>
          </div>
      ...
      <section className="mx-auto grid max-w-[1920px] gap-5 px-6 py-2 2xl:px-8 xl:grid-cols-[0.3fr_0.24fr_0.46fr]">
        ...
        <div className="rounded-xl border bg-card p-7 shadow-sm"><h2 className="text-center font-display text-[28px] font-extrabold text-brand-navy">How Much Do You Need</h2><p className="text-center text-xl font-extrabold text-muted-foreground">Start small. Grow big.</p><div className="mt-8 grid gap-4 lg:grid-cols-4">{plans.map(([name, price, suffix, sub, account, popular])=><article key={name as string} className={`relative rounded-xl border bg-card p-5 text-center shadow-sm transition-all hover:-translate-y-2 hover:border-primary hover:shadow-card ${popular ? "border-primary shadow-card" : ""}`}>{popular && <span className="absolute -top-4 left-1/2 -translate-x-1/2 rounded-full bg-success px-5 py-1 text-xs font-extrabold text-success-foreground">Most Popular</span>}<h3 className="min-h-[52px] text-lg font-extrabold leading-7 text-primary">{name as string}</h3><p className="mt-5 break-words font-display text-[clamp(2rem,2.2vw,2.55rem)] font-extrabold leading-none text-brand-navy">{price as string} <span className="text-sm font-bold text-muted-foreground">{suffix as string}</span></p><p className="mt-5 whitespace-pre-line text-sm font-bold leading-6 text-muted-foreground">{sub as string}</p><p className="mt-8 flex items-center justify-center gap-2 text-base font-extrabold leading-6 text-brand-navy"><Check className="size-5 shrink-0 text-success" />{account as string}</p></article>)}</div><div className="mt-6 grid gap-4 md:grid-cols-4">{[[BadgeIndianRupee,"No Hidden Fees"],[Clock3,"Cancel Anytime"],[Headphones,"24/7 Support"],[UsersRound,"Secure & Trusted"]].map(([Icon,label])=>{const PlanIcon=Icon as typeof ShieldCheck; return <div key={label as string} className="flex min-h-[76px] items-center justify-center gap-3 rounded-lg border bg-secondary/50 px-4 py-3 text-sm font-extrabold leading-5 text-brand-navy"><PlanIcon className="size-7 shrink-0 text-primary" />{label as string}</div>})}</div></div>
      </section>
